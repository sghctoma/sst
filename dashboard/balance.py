...
def _balance_data(front_strokes: list[Stroke], rear_strokes: list[Stroke],
    ...
    f = dict(travel=ft, velocity=fv, trend=[fp(t) for t in ft])
    r = dict(travel=rt, velocity=rv, trend=[rp(t) for t in rt])
...
def balance_figure(front_strokes: list[Stroke], rear_strokes: list[Stroke],
    ...
    front_source = ColumnDataSource(data=f)
    rear_source = ColumnDataSource(data=r)
    ...
    p.x_range = Range1d(
        0, np.fmax(f['travel'][-1], r['travel'][-1]))
