...
def on_selectiongeometry(event):
    ...
    mask = np.full(len(telemetry.Front.Travel if telemetry.Front.Present else telemetry.Rear.Travel), False)
    mask[start:end] = True
...
'''
comb_topouts = combined_topouts(front_travel if telemetry.Front.Present else np.full(len(rear_travel), 0),
    telemetry.Front.Calibration.MaxStroke,
    rear_travel if telemetry.Rear.Present else np.full(len(front_travel), 0),
    telemetry.Frame.MaxRearTravel,
    telemetry.SampleRate)
airtimes = filter_airtimes(comb_topouts,
    front_velocity if telemetry.Front.Present else np.full(len(rear_velocity), 0),
    rear_velocity if telemetry.Rear.Present else np.full(len(front_velocity), 0),
    telemetry.SampleRate)
airtimes_mask = intervals_mask(np.array(airtimes), len(front_travel if telemetry.Front.Present else rear_travel), False)
